			...
			while (true) {
				...
				if(lblHorse.getX()==540) {
					...
					if(index == horses.length-1) {
						JOptionPane.showMessageDialog(RaceDuckFrame.this, (winnerIndex[0]+1)+"말이 우승!!!");
						if(winnerIndex[0]==bet1.getBetingIndex())
							JOptionPane.showMessageDialog(RaceDuckFrame.this, "축하합니다. "+bet1.getName()+"님 배팅에 성공하였습니다.");
						else if(winnerIndex[0]==bet2.getBetingIndex())
							JOptionPane.showMessageDialog(RaceDuckFrame.this, "축하합니다. "+bet2.getName()+"님 배팅에 성공하였습니다.");
						else
							JOptionPane.showMessageDialog(RaceDuckFrame.this, "다음에 다시 배팅 부탁드려요~. 모두 배팅에 실패하였습니다.");
						index = 0;
						for (int i = 0; i < horses.length; i++) {
//							System.out.println(winnerIndex[i]);
							horses[i].setLocation(0, horses[i].getY());
							horses[i].setIcon(new ImageIcon("images/an"+(i+1)+".gif"));
						}
					}
					break;
				}
